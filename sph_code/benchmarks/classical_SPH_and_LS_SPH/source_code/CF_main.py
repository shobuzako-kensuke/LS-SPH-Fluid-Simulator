...
import CF_initial_settings as ini
import CF_V_rms as V_rms
import CF_uv_plot as uv_plot
import CF_snapshot as snap
...
if (exe_V_rms=='y'):
    ...
    os.makedirs(save_path, exist_ok=True) # mkdir
    V_rms.main(N_sys, N_inn, N_file, last_file, write_step, Delta_t_eff, U_top, parent_path, save_path)
...
if (exe_uv_plot=='y'):
    ...
    save_path = '../fig/{}'.format(save_name)
    os.makedirs(save_path, exist_ok=True) # mkdir
    uv_plot.main(kernel_switch, SPH_model, N_sys, WL_thick, last_file, U_top, SP_mass, h, Re, parent_path, save_path)
