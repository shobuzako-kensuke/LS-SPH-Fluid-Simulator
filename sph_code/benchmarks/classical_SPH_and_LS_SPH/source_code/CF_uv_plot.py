...
#  main                   # 
#=========================#
def main(kernel_switch, SPH_model, N_sys, WL_thick, last_file, U_top, SP_mass, h, Re, parent_path, save_path):
    #=========================#
    #  read                   # 
    #=========================#
    file_name = parent_path + '/data/{}/SP_xyz.dat'.format(last_file) # SP_xyz
    SP_xyz    = read_f90.binary_files(file_name, N_sys, 2)
    SP_xyz    = SP_xyz[:,:] - WL_thick                                # tranlation
    ...
    #  figure                 # 
    #=========================#
    ref = np.loadtxt('../reference_solution/Ghia et al. (1982) Re_{}.csv'.format(int(Re)), \
                     delimiter=',', dtype='float64', skiprows=1) # read reference
    ...
    # plot
    ax.plot(MP_uvw[:N_cal, 0], MP_xyz[:N_cal, 1], c='r')
    ax.scatter(ref[:, 1], ref[:, 0], label='Ghia et al. (1982)', c='k', marker='s', s=20)
    # axis label
    ...
    # plot
    ax.plot(MP_xyz[N_cal:, 0], MP_uvw[N_cal:, 1], c='r', linewidth=2.0)
    ax.scatter(ref[:, 2], ref[:, 3], label='Ghia et al. (1982)', c='k', marker='s', s=20)
    # axis label
