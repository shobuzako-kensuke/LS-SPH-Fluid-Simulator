...
#  fig_cbar_yticks        # 
#=========================#
def fig_cbar_yticks(Pr, Ra, val, cbar, ax):
    if (Pr==0.71):
        if (val=='u'):
            cbar.ax.set_yticks([-0.4, -0.2, 0, 0.2, 0.4])
            cbar.ax.set_yticklabels(['$-0.4$', '$-0.2$', '$0.0$', '$0.2$', '$0.4$'])
        if (val=='v'):
            cbar.ax.set_yticks([-0.4, -0.2, 0, 0.2, 0.4])
            cbar.ax.set_yticklabels(['$-0.4$', '$-0.2$', '$0.0$', '$0.2$', '$0.4$'])
        if (val=='UV'):
            cbar.ax.set_yticks([0, 0.1, 0.2])
            cbar.ax.set_yticklabels(['$0.0$', '$0.1$', '$0.2$'])
        if (val=='rho'):
            cbar.ax.set_yticks([0.9, 1, 1.1])
            cbar.ax.set_yticklabels(['$0.9$', '$1.0$', '$1.1$'])
        if (val=='pre'):
            cbar.ax.set_yticks([-1.0, 0, 1.0])
...
#  fig_detail             # 
#=========================#
def fig_detail(Pr, Ra, val, fig, lim_switch, boudary_switch, time, step, SP_xyz, SP_val, val_min, val_max, sca_size):
    ...
    fig_cbar_yticks(Pr, Ra, val, cbar, ax)
...
def main(N_sys, N_inn, WL_thick, N_file, last_file, write_step, \
    ...
    for i in range(0, last_file+1, write_step):
        ...
        if (exe_fig_pre=='y'):
            ...
            SP_pre   /= (rho_ref* alpha* Delta_temp* gravity* height)  # normalize
            if (count==1):
                save_path = '../fig/{}/snapshot_pre'.format(save_name)
                os.makedirs(save_path, exist_ok=True)                  # mkdir for pre

        if (exe_fig_tem=='y'):
            file_name = parent_path + '/data/{}/SP_tem.dat'.format(i)  # SP_tem
            SP_tem    = read_f90.binary_files(file_name, N_sys, 1)
            SP_tem[:,0] = (SP_tem[:,0] - temp_ave) / Delta_temp        # normalize
            if (count==1):
                save_path = '../fig/{}/snapshot_tem'.format(save_name)
                os.makedirs(save_path, exist_ok=True)                  # mkdir for tem
        ...
        if (exe_fig_u=='y'):
            ...
            # details
            if (add_wall=='y'):
                fig_detail(Pr, Ra, 'u', fig, 0, 1, time, i, SP_xyz, SP_uvw[:,0], u_min, u_max, sca_size)
            else:
                fig_detail(Pr, Ra, 'u', fig, 1, 0, time, i, SP_xyz, SP_uvw[:,0], u_min, u_max, sca_size)
            # save
        ...
        if (exe_fig_v=='y'):
            ...
            # details
            if (add_wall=='y'):
                fig_detail(Pr, Ra, 'v', fig, 0, 1, time, i, SP_xyz, SP_uvw[:,1], v_min, v_max, sca_size)
            else:
                fig_detail(Pr, Ra, 'v', fig, 1, 0, time, i, SP_xyz, SP_uvw[:,1], v_min, v_max, sca_size)
            # save
        ...
        if (exe_fig_UV=='y'):
            ...
            # details
            if (add_wall=='y'):
                fig_detail(Pr, Ra, 'UV', fig, 0, 1, time, i, SP_xyz, SP_UV, UV_min, UV_max, sca_size)
            else:
                fig_detail(Pr, Ra, 'UV', fig, 1, 0, time, i, SP_xyz, SP_UV, UV_min, UV_max, sca_size)
            # save
        ...
        if (exe_fig_rho=='y'):
            ...
            # details
            if (add_wall=='y'):
                fig_detail(Pr, Ra, 'rho', fig, 0, 1, time, i, SP_xyz, SP_rho, rho_min, rho_max, sca_size)
            else:
                fig_detail(Pr, Ra, 'rho', fig, 1, 0, time, i, SP_xyz, SP_rho, rho_min, rho_max, sca_size)
            # save
        ...
        if (exe_fig_pre=='y'):
            ...
            # details
            if (add_wall=='y'):
                fig_detail(Pr, Ra, 'pre', fig, 0, 1, time, i, SP_xyz, SP_pre, pre_min, pre_max, sca_size)
            else:
                fig_detail(Pr, Ra, 'pre', fig, 1, 0, time, i, SP_xyz, SP_pre, pre_min, pre_max, sca_size)
            # save
        ...
        if (exe_fig_tem=='y'):
            ...
            # details
            if (add_wall=='y'):
                fig_detail(Pr, Ra, 'tem', fig, 0, 1, time, i, SP_xyz, SP_tem, tem_min, tem_max, sca_size)
            else:
                fig_detail(Pr, Ra, 'tem', fig, 1, 0, time, i, SP_xyz, SP_tem, tem_min, tem_max, sca_size)
            # save
            fig.savefig('../fig/{}/snapshot_tem/{}.png'.format(save_name, i), format='png', dpi=300, transparent=False)
            plt.close()
    ...
    if (exe_fig_pre=='y'):
        ...
        print('finish')
    if (exe_fig_tem=='y'):
        print('          making a movie for tem ... ', end='', flush=True)
        mk_movie.func_animation(last_file, write_step, save_name, 'tem')
        print('finish')
